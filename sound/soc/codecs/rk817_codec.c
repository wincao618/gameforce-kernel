				...
				int gpio, int level)
{
	...
	if ((gpio & CODEC_SET_SPK) &&
	    rk817->spk_ctl_gpio) {
		DBG("%s set mode=%d spk clt %d\n", __func__, AW8733_WORKMODE, level);
        if (level) {
...
static struct rk817_reg_val_typ playback_power_down_list[] = {
	...
	/* HP */
	{RK817_CODEC_AHP_CFG0, 0x80},//e0 80
	{RK817_CODEC_AHP_CP, 0x11},//09 11
	/* SPK */
	{RK817_CODEC_ACLASSD_CFG1, 0x69},
};
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	case SPK_PATH:
		...
		rk817_codec_ctl_gpio(rk817, CODEC_SET_SPK, 1);//open speaker
#ifndef CONFIG_ARCH_ROCKCHIP_ODROIDGO2
	...
	case HP_PATH:
		...
		rk817_codec_ctl_gpio(rk817, CODEC_SET_SPK, 0);//close speaker
#ifndef CONFIG_ARCH_ROCKCHIP_ODROIDGO2
